...
const StatCard = ({ label, value, change, up, icon: Icon }: any) => (
  <div className="bg-white border border-slate-100 p-8 rounded-[2rem] hover:shadow-2xl transition-all group">
    <div className="flex justify-between items-start mb-6">
      ...
      <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black ${up ? 'bg-cyan-50 text-cyan-600' : 'bg-rose-50 text-rose-600'}`}>
    ...
    <div className="space-y-10 pb-12">
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 bg-white border border-slate-100 p-10 rounded-[3rem] shadow-sm">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1 bg-cyan-50 rounded-lg">
                <span className="w-2 h-2 bg-cyan-500 rounded-full" />
                <span className="text-[10px] font-black uppercase text-cyan-600">Price Trend</span>
              </div>
                      ...
                      return (
                        <div className="bg-slate-900 text-white p-5 rounded-[1.5rem] shadow-2xl border border-slate-800 min-w-[200px]">
                          <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-800">
                             ...
                             <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${data.sentiment === 'Bullish' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-rose-500/20 text-rose-400'}`}>
                               {data.sentiment}
                             </span>
                          </div>
        ...
        <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col relative overflow-hidden">
          <div className="absolute -top-10 -right-10 w-64 h-64 bg-cyan-500/10 rounded-full blur-[80px]" />
          ...
          <div className="space-y-8 flex-1 relative z-10">
            {[
              { label: 'Kerala Hub (IN)', val: '45%', color: 'bg-cyan-500', trend: '+2.4%' },
              { label: 'Dak Lak Node (VN)', val: '24%', color: 'bg-cyan-600', trend: '-1.1%' },
              { label: 'Espírito Santo (BR)', val: '18%', color: 'bg-cyan-700', trend: '+0.8%' },
              ...
              <div key={origin.label} className="space-y-3">
                <div className="flex justify-between text-[10px] font-black tracking-[0.2em] uppercase">
                  ...
                  <div className="flex gap-3">
                    <span className={origin.trend.startsWith('+') ? 'text-cyan-400' : 'text-rose-400'}>{origin.trend}</span>
          ...
          <div className="mt-12 pt-8 border-t border-slate-800">
             <div className="flex items-center gap-4 p-5 bg-white/5 rounded-[2rem] border border-white/5 group hover:bg-white/10 transition-colors cursor-pointer">
               <div className="p-3 bg-cyan-500/20 text-cyan-500 rounded-xl group-hover:scale-110 transition-transform">
                <BarChart3 size={24} />
               </div>
               <div>
